...
`/model init` 会在 `.codemate/models.properties` 创建配置模板。该文件已被 Git 忽略，真实 API Key 只应填写在该本地文件或 `.env` 中。可通过模型名（如 `/model grok-4.5`）或档案名（如 `/model ccswitch`）切换；模型切换后会重建 LLM Client，并清空当前会话上下文。
